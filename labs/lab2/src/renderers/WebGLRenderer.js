        ...
        for (let l = 0; l < this.lights.length; l++) {
            ...
            for (let i = 0; i < this.meshes.length; i++) {
                ...
                for (let k in this.meshes[i].material.uniforms) {
                    ...
                    let precomputeL_RGBMat3 = getRotationPrecomputeL(precomputeL[guiParams.envmapId], cameraModelMatrix); // precomputeL 9 * 3
                    let Mat3Value = getMat3ValueFromRGB(precomputeL_RGBMat3) // Mat3Value 3(color channel) * 3 * 3 
                    for(let j = 0; j < 3; j++){
                        if (k == 'uPrecomputeL['+j+']') {
                            gl.uniformMatrix3fv(
                                this.meshes[i].shader.program.uniforms[k],
